  ...
  useEffect(() => {
    const fetchTrendData = async () => {
      try {
        ...
        const trendRes = await api.get("/api/energy/trends");
        setConsumptionTrends(trendRes.data?.trends || []);

        // ✅ 2. Fetch AI predictions
        const aiRes = await api.get("/api/ai/predictions");
        setAiPrediction(aiRes.data || {});

        // ✅ 3. Fetch historical energy data
        const histRes = await api.get("/api/energy/history");
        setHistoricalData(histRes.data?.history || []);

        // ✅ 4. Fetch insights (optional)
        const insightRes = await api.get("/api/energy/insights");
